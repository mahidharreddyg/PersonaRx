...
def schedule():
    try:
        ...
        schedule_list = generate_schedule(medications, meal_times)
...
def simulate():
    ...
    try:
        ...
        if not schedule_list:
            return jsonify({"error": "No schedule found. Call /schedule first."}), 404
...
def replan():
    ...
    {
        "session_id": str,
        "updated_schedule": [ dose_dicts ],
        "summary": { persona, confidence, reasoning, adherence, replanned_doses, actions }
    }
    """
    try:
        data       = request.get_json(force=True)
        ...
        # infer persona
        persona_result = infer_persona(events)
        ...
        # replan
        updated_schedule = replan_schedule(schedule_list, events, meal_times)

        # compute adherence
        from simulator import compute_adherence
        adherence = compute_adherence(events)

        # build summary
        summary = build_planning_summary(schedule_list, updated_schedule, persona_result, adherence)
