...
def get_schedule(session_id: str) -> list:
    ...
    try:
        ...
        doc = schedules_col.find_one({"session_id": session_id}, {"_id": 0})
        if not doc:
            return []
        return doc.get("medications") or doc.get("schedule", [])
    except Exception:
        return []
...
def save_event(event: dict, session_id: str):
    if not _db_available:
        return
    try:
        # Create a copy to avoid mutating the original dict used by the Flask response
        event_copy = event.copy()
        event_copy["session_id"] = session_id
        event_copy.pop("_id", None)
        events_col.insert_one(event_copy)
    except Exception:
        pass
...
def get_state(session_id: str) -> dict:
    if _db_available:
        try:
            doc = state_col.find_one({"session_id": session_id}, {"_id": 0})
            if doc:
                return doc
        except Exception:
            pass
