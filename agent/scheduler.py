...
def adjust_meal_times(current_meal_times: dict, persona: str) -> dict:
    ...
    if persona == "forgetful":
        # push all times 60 min later — forgetful users tend to be late
        updated["breakfast"] = shift(updated["breakfast"], +60)
        updated["lunch"]     = shift(updated["lunch"],     +60)
        updated["dinner"]    = shift(updated["dinner"],    +60)

    elif persona == "busy":
        # skip lunch (handled by inference) and push dinner later
        updated["dinner"] = shift(updated["dinner"], +60)
